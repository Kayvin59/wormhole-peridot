...
export default function ChainDropdown() {
  ...
  function refreshChainData() {
    if (isConnected === true) {
      Promise.all([getChainId(), getChainName()]).then((values) => {
        setChainId(values[0]);
  ...
  function getChainButtons() {
    ...
    const chainButtons = chains.map((chain, index) => {
      ...
      let title =
        isConnected !== true ? "please connect your wallet first" : undefined;

      return (
        <button
          key={index}
          className={conc(styles.chain_button, styles.chain_image_container)}
          onClick={() => handleSwitchChainButton(chain)}
          disabled={isDisabled}
          title={title}
        >
          <img
            className={styles.chain_image}
            src={"/images/chains/" + chain.nameId + ".svg"}
            alt={
              chain.name + " (" + chain.nameId + ") chain for " + projectName
            }
          />
  ...
  return (
    <div
      ...
      onMouseLeave={() => setIsChainsOpen(false)}
    >
      <button
        className={conc(
          styles.dropdown_button,
          cond(isChainsOpen, styles.dropdown_button_active),
        )}
        onMouseEnter={() => setIsChainsOpen(true)}
        ...
        {chainName !== undefined ? (
          <div className={styles.chain_image_container}>
            {isChainImage === true && (
              <img
                className={styles.chain_image}
                src={"/images/chains/" + chainName + ".svg"}
                alt={chainName + " chain for " + projectName}
                onError={() => setIsChainImage(false)}
              />
            )}
            <span>{chainName}</span>
          </div>
      ...
      {isChainsOpen && (
        <ul className={styles.chain_button_container}>{getChainButtons()}</ul>
      )}
    </div>
  );
}
