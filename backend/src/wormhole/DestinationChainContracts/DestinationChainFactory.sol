...
contract DestinationChainFactory is IWormholeReceiver, Ownable {
    ...
    mapping(address => string) public fTTNames;
    ...
    function receiveWormholeMessages(
        ...
        (address sourceFFT, string memory name) = abi.decode(payload, (address, string));

        fTTNames[sourceFFT] = name;
     ...
     * @param sourceFTT The address of the source FFT contract.
     * @return wrappedFFT The address of the deployed WrappedFFT token.
     */
    function _deployWrappedFFT(string memory name, string memory symbol, address sourceFTT) external onlyOwner returns (address wrappedFFT) {
