...
contract PeridotSwap is ERC721Holder, ERC1155Holder, Ownable, IPeridotSwap, ReentrancyGuard {
    ...
    constructor(address _witnetRandomness) Ownable() {
  ...
  function swapMiniNFTtoNFT(address NFTContract) external payable nonReentrant returns (bool) {
    ...
    uint256 fee = witnetRandomness.randomize{value: msg.value}();
    require(fee <= msg.value, "swap: fee exceeds msg.value");
    ...
    uint256 requestId = uint256(keccak256(abi.encodePacked(msg.sender, block.number)));
  ...
  function processRandomness() external {
    ...
    uint32 randomNumber = witnetRandomness.random(type(uint32).max, request.nonce, request.requestBlock);
    
    /*bytes32 randomness = witnetRandomness.fetchRandomnessAfter(request.requestBlock);
    uint256 randomnessUint = uint256(randomness); // Convert bytes32 to uint256
    uint32 randomNumber = witnetRandomness.random(type(uint32).max, request.nonce, randomnessUint);
    */
...
function swapFFTtoMiniNFT(address miniNFTContract, uint256 miniNFTAmount)
  ...
  */
  //Removed onlyDao modifier
  function getLatestRequestId(address user) external view returns  (uint256) {
      return latestRequestById[user];
  }

  function updateOracle(address newOracle) external onlyDAO returns (bool) {
    witnetRandomness = IWitnetRandomness(newOracle);
    return true;
  }
