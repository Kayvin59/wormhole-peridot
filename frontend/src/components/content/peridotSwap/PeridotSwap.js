...
import {
  ...
  swapMiniNftToToken,
  swapTokenToMiniNft,
} from "../../../lib/web3/web3PeridotSwap.js";
...
export default function PeridotSwap() {
  ...
  const urlChain =
    urlParams.get("chain") !== null ? urlParams.get("chain") : undefined;
  const urlName =
    urlParams.get("name") !== null ? urlParams.get("name") : undefined;
  const urlInput =
    urlParams.get("input") !== null ? urlParams.get("input") : undefined;
  ...
  function isIntegerToken(token) {
    return token !== undefined
      ? token.contract !== collection.token.contract
      : false;
  }
    ...
    if (isDefined(topToken, bottomToken)) {
      if (topValue !== "" && parseFloat(topValue) !== 0) {
        if (topValue !== oldTopValue) {
          ...
          let isTokenBottom =
            bottomToken.contract === collection.token.contract;
    ...
    if (isDefined(topToken, bottomToken)) {
      if (bottomValue !== "" && parseFloat(bottomValue) !== 0) {
        if (bottomValue !== oldBottomValue) {
          ...
          getTopValue(isTokenBottom, tax, bottomValue).then((result) => {
  ...
  function getTokenModalElements(isTop) {
    if (isDefined(topToken, bottomToken)) {
      let tokens = [collection.nft, collection.miniNft, collection.token];
      ...
      return tokens.map((token, index) => {
        ...
        let isSelected =
          token.chain === selectedToken.chain &&
          token.symbol === selectedToken.symbol;

        let classes = conc(
          styles.token_modal_element,
          cond(isSelected, styles.token_modal_element_selected),
        );

        let onClick = isTop
          ? () => handleTopTokenModalSelect(token)
          : () => handleBottomTokenModalSelect(token);
    ...
    <SectionContainer>
      <Section title={"Swap Tokens"}>
        <div>
            ...
            <div className={styles.swap_box_container}>
              ...
              <div className={styles.token_container}>
                ...
                <div className={styles.input_footer}>
                  ...
                  <div
                    className={styles.input_token_container}
                    onClick={() => setIsTopTokenModalOpen(true)}
                  >
              ...
              <div className={styles.token_container}>
                <div className={styles.input_header}>
                  <div>to</div>
                  {isDefined(bottomToken, collection) &&
                    bottomToken.contract !== collection.nft.contract && (
                      <div>
                        <div className={styles.balance_container}>
                          <span>Balance:</span>
                ...
                <div className={styles.input_footer}>
                  ...
                  <div
                    className={styles.input_token_container}
                    onClick={() => setIsBottomTokenModalOpen(true)}
                  >
          ...
          <Modal
            isOpen={isTopTokenModalOpen}
            close={() => setIsTopTokenModalOpen(false)}
            title={"Select Top Token"}
          >
          ...
          <Modal
            isOpen={isBottomTokenModalOpen}
            close={() => setIsBottomTokenModalOpen(false)}
            title={"Select Bottom Token"}
          >
            <div className={styles.token_modal_container}>
