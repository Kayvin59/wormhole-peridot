...
export async function claimFTT(miniNftAddress) {
	let ifoContract = new web3Provider.Contract(wormholeIFOAbi, contracts.ARB.help.ifo);

	return ifoContract.methods.claimWrappedFTT(miniNftAddress).send(getSendMessage());
}