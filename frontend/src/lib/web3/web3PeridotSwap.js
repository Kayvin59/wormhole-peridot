...
export async function approveNft(miniNftContract, swapAddress) {
  return miniNftContract.methods
    .setApprovalForAll(swapAddress, true)
    .send(getSendMessage());
}
...
export async function swapMiniNftToNft(nftAddress, miniNftAddress) {
  ...
  let witnetContract = new web3Provider.Contract(
    witnetAbi,
    contracts[chain].witnet,
  );
  ...
  return approveNft(miniNftContract, contracts[chain].swap).then(async () => {
    ...
    return witnetContract.methods
      .LONG_WITNET_CALL(nftAddress)
      .send(getSendMessage())
      .then(() => {
        // TODO
        return swapContract.methods
          .swapMiniNFTtoNFT(nftAddress)
...
export async function swapMiniNftToToken(miniNftAddress, inputAmount) {
  ...
  return approveNft(miniNftContract, contracts[chain].swap).then(() => {
    ...
    return swapContract.methods
      .swapMiniNFTtoFFT(miniNftContract, blindBoxId, inputAmount)
      .send(getSendMessage());
  });
}

export async function swapTokenToMiniNft(
  tokenAddress,
  miniNftAddress,
  inputAmount,
  outputAmount,
  ...
  return approve(tokenContract, contracts[chain].swap, weiInputAmount).then(
    () => {
      return swapContract.methods
        .swapFFTtoMiniNFT(miniNftAddress, outputAmount)
        .send(getSendMessage());
    },
  );
}
